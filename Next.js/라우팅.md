...
> [!warning] 라우트 간에 내비게이션하려면 `<Link>` 컴포넌트를 사용하는게 좋습니다.
# `error.js`파일로 오류 처리
...
# 병렬 라우트
- **슬롯**(병렬 라우트를 부르는 용어): `@폴더명`으로 정의하며, **동일한 레벨의 `layout`에 prop으로 전달됩니다.**
- 슬롯은 URL로 인식되지 않습니다. -> 세그먼트로 취급하지 않습니다.
	![[parallel-route1.png]]
...
- 사용자가 다른 컨텍스트로 전환하지 않고도 라우트의 콘텐츠를 표시하고 싶을 때 유용합니다. -> **로그인 페이지를 모달로 만들고 라우트 가로채기를 하면, 로그인 후 마지막 페이지로 돌아갈때 모달을 닫기만 하면 됨**
...
- 들어오는 요청을 기반으로 응답을 재작성, 리디렉션, 요청 또는 응답 헤더 수정하거나, 직접 응답하는 방식으로 수정할 수 있습니다.
