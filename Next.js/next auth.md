...
- `jwt()` 콜백은 반드시 `token`을 반환해야 합니다. 반환값은 암호화되
- 로그인 시 `token`은 다음과 같이 구성됩니다:
	...
	```
	- sub: 데이터베이스에 저장된 로우의 id 값입니다.
	- `jwt({ token })`의 `token`은 `session({ token })`의 `token`과 완전히 동일합니다.

- `user`에는 정확히 어떻게 어떤 값이 저장되는 건지 모릅니다. 추측으로는 로그인되는 순간에만 사용자 정보를 저장하고 그 외에는 `undefined`인 것 같지만, 현재로써는 믿고 사용할 수 없습니다.
- `profile`도 `user`와 같은 이유로 현재로써는 사용할 수 없습니다.