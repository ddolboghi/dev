...
hello("naen"); // naen, hello!
howdy("naen"); // naen, how are you?
```
1. `greeting()`이 실행되고 `who()`의 인스턴스가 생성된다.
2. `who()`는 `greeting()`이 있는 외부 스코프에서 넘어온 매개변수 msg를 **close over**한다.
3. `greeting()`이 호출되면 `who()`가 반환되고, `who()`에 대한 참조는 외부 스코프에 있는 변수(`hello`, `howdy`)에 할당 된다. 그래서 변수에 `who()`의 인자를 함수 형태로 전달할 수 있는 것이다.
4. 두번째 `greeting()`이 호출되면 새로운 내부 함수`who()`의 인스턴스가 생성된다.

- `greeting()`이 호출된 이후에도 `msg`는 여전히 `who()`에서 close over하고 있고, `who()`의 참조가 외부변수에 할당되있기 때문에 `msg`는 가비지 컬렉션의 대상이 되지 않는다.
- 클로저는 

> [!NOTE]
> close over: 외부 스코프에 있는 변수를 기억하는 현상. 클로저가 해당 변수를 자신의 환경에 포함하고, 그 변수를 계속해서 접근 가능하게 유지한다.