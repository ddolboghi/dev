...
## 패킷 구성과 캡슐화
- 패킷 1개는 최대 1500bite이며, 이를 MTU(Maximum Transport Unit)라고 한다.
- 패킷은 header와 payload로 구성된다.
	- header는 [IP | TCP] 각각 일반적으로 20bite로 이뤄져있다.
	- 이때 payload는 1460bite이며, 이를 MSS(Maximum Segment Size)라고 한다.
...
- L2 distribution는 일반적으로 게이트웨이 역할을하는 router로 연결된다.
- router는 인터넷과 연결된다.
> [!info]
> - uplink: 상위 계층의 스위치로 연결되는 라인

> [!info]
> - Link-up: LAN 케이블에 연결됨
> - Link-down: LAN 케이블 연결 끊김
