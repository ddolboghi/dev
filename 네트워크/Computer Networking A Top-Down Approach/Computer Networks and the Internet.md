...
- 패킷 스니핑 소프트웨어는 웹 사이트와 상업 제품으로 자유롭게 이용 가능하며, Wireshark [Wireshark 2020]와 같은 패킷 스니퍼가 이 교재의 Wireshark 실습과 관련되어 있습니다 [95, 117, 118, Figure 1.28].

◦

패킷 스니퍼는 수동적이므로(채널에 패킷을 주입하지 않음) 탐지하기 어렵습니다1718. 따라서 무선 채널로 패킷을 보낼 때, 악당이 패킷 사본을 기록할 가능성을 받아들여야 합니다1718. 패킷 스니핑에 대한 최선의 방어책 중 일부는 **암호화(cryptography)**를 포함하며, 이는 8장에서 자세히 다루어질 예정입니다1718.
...
임의의 소스 주소, 패킷 내용 등으로 패킷을 생성하는 것이 놀랍도록 쉽습니다17. 이는 한 사용자가 다른 사용자로 가장할 수 있는 다양한 방법을 가능하게 합니다19.

◦

이 문제를 해결하기 위해서는 종단점 인증(end-point authentication), 즉 메시지가 우리가 생각하는 곳에서 실제로 originated했는지 확실하게 판단할 수 있는 메커니즘이 필요합니다1920. 이 역시 8장에서 자세히 탐구됩니다1920.
