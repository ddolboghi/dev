...
# IP-Anycast
BGP는 인터넷의 AS 간 라우팅 프로토콜일 뿐만 아니라, DNS에서 흔히 사용되는 **IP-anycast** 서비스를 구현하는 데에도 자주 사용된다.

## IP-Anycast의 개념과 동기

많은 애플리케이션에서는 동일한 콘텐츠를 지리적으로 분산된 여러 서버에 복제하고, 각 사용자는 가장 가까운 서버에서 콘텐츠에 접근하도록 한다. (e.g. CDN, DNS 시스템은 DNS 레코드를 전 세계 서버에 복제할 수 있다.

### BGP를 이용한 IP-Anycast 구현

IP-anycast는 BGP의 경로 선택 알고리즘을 통해 자연스럽게 구현될 수 있다.

1. **동일 IP 주소 할당**: CDN 회사는 지리적으로 분산된 각 서버에 **동일한 IP 주소**를 할당한다.
    
2. **BGP 경로 광고**: 각 서버는 표준 BGP를 사용하여 이 동일한 IP 주소에 대한 경로를 광고한다.
    
3. **경로 선택**: BGP 라우터가 이 IP 주소에 대한 여러 경로 광고를 수신하면, 이들을 서로 다른 물리적 위치가 아닌 동일한 물리적 위치로 가는 다른 경로로 취급한다. 각 라우터는 로컬에서 BGP 경로 선택 알고리즘을 사용하여 해당 IP 주소로 가는 "최상의" 경로(예: AS-hop 카운트 기준 가장 가까운 경로)를 선택하고 자신의 라우팅 테이블에 설정한다.
    
4. **라우팅**: 클라이언트가 해당 IP 주소로 요청을 보내면, 인터넷 라우터들은 BGP 경로 선택 알고리즘에 의해 결정된 "가장 가까운" 서버로 요청 패킷을 포워딩한다.
    

이 방식은 Figure 5.12에 설명되어 있다.

### IP-Anycast의 사용 사례

CDN의 예시가 IP-anycast를 잘 설명하지만, 실제 CDN은 BGP 라우팅 변경으로 인해 동일한 TCP 연결의 패킷들이 서로 다른 서버 인스턴스에 도착할 수 있기 때문에 일반적으로 IP-anycast를 사용하지 않는다.

그러나 IP-anycast는 DNS 시스템에서 DNS 쿼리를 가장 가까운 루트 DNS 서버로 보내기 위해 광범위하게 사용된다. 13개의 루트 DNS 서버 IP 주소 각각에 대해 전 세계에 흩어져 있는 수많은 DNS 루트 서버들이 존재하며, IP-anycast는 쿼리를 이들 중 가장 가까운 서버로 라우팅한다.
