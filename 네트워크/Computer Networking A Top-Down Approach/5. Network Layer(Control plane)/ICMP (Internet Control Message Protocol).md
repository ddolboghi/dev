...
- 라우터는 ICMP 메시지를 생성하여 호스트에 오류 정보를 송신한다.
...
### ICMP 메시지 타입
ICMP는 오류 보고 뿐만아니라, 진단 목적으로도 사용된다.(ping 프로그램의 타입 8, 0)
...
- 위 방식들로 출발지 호스트는 목적지까지의 라우터 수와 ID, 두 호스트 간의 왕복 시간을 학습한다.
# ICMPv6
- IPv6 용으로 정의된 ICMP다.
- 기존 ICMP 타입 및 코드를 재정의하고 IPv6에 필요한 새 타입과 코드를 추가했다.
	- packet too big 타입, 오류 코드 unrecognized IPv6 options
