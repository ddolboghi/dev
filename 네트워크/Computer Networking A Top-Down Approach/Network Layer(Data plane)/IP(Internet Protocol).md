...
- **플로우 레이블링(Flow labeling)**: 20비트의 플로우 레이블 필드를 통해 특정 플로우에 속한 패킷들을 식별할 수 있게 했다. 이는 실시간 서비스나 특정 QoS(서비스 품질)를 요구하는 트래픽을 처리하는 데 유용하다.
### IPv6 헤더 필드
- **Version**: 4비트 필드로, IPv6의 경우 값이 6이다.
- **Traffic class**: 8비트 필드로, IPv4의 TOS 필드처럼 데이터그램의 우선순위를 지정하는 데 사용된다.
- **Flow label**: 20비트 필드로, 특정 플로우에 속한 데이터그램들을 식별한다.
- **Payload length**: 16비트 필드로, 40바이트 고정 헤더를 제외한 데이터그램 페이로드의 길이를 바이트 단위로 나타낸다.
    
- **Next header (다음 헤더)**: 페이로드에 담긴 데이터가 전달될 상위 프로토콜(예: TCP 또는 UDP)을 식별한다. IPv4의 프로토콜 필드와 동일한 값을 사용한다.
    
- **Hop limit (홉 제한)**: 데이터그램을 전달하는 각 라우터가 1씩 감소시키며, 값이 0이 되면 데이터그램을 폐기한다. IPv4의 TTL 필드와 유사하다.
    
- **Source and destination addresses (출발지 및 목적지 주소)**: 각각 128비트 길이의 주소이다.
    
- **Data (데이터)**: 데이터그램의 페이로드 부분이다.
...
- **Header checksum (헤더 체크섬)**: 전송 계층과 링크 계층에서 이미 오류 검사를 수행하므로, 네트워크 계층에서의 체크섬 기능은 중복으로 간주되어 제거되었다. 이 또한 라우터의 처리 속도를 높이는 데 기여했다.
    
- **Options (옵션)**: 표준 IP 헤더에서 제거되었지만, '다음 헤더' 필드를 통해 확장 헤더로 포함될 수 있다. 이로 인해 IP 헤더는 40바이트 고정 길이를 갖게 되었다.
    

### IPv4에서 IPv6로의 전환

모든 인터넷 장비를 한 번에 IPv6로 전환하는 것은 불가능하므로, **터널링(tunneling)**이라는 점진적인 전환 방식이 채택되었다. 터널링은 Figure 4.27에 설명되어 있다.

- IPv6 노드 간에 IPv4 라우터들이 존재하는 경우, 출발지 IPv6 노드는 전체 IPv6 데이터그램을 IPv4 데이터그램의 페이로드 부분에 넣는다.
    
- 이 IPv4 데이터그램은 터널의 반대편 IPv6 노드를 목적지로 하여 전송된다.
    
- 중간의 IPv4 라우터들은 이것을 일반적인 IPv4 데이터그램으로 처리한다.
    
- 목적지 IPv6 노드는 IPv4 데이터그램을 수신하면, 페이로드에서 IPv6 데이터그램을 추출하여 정상적으로 처리한다.