...
- **NAT 변환 테이블**: 외부 인터넷으로 나가는 모든 트래픽은 NAT 라우터의 공인 IP 주소(e.g. `138.76.29.7`)를 출발지 IP 주소로 사용한다. 라우터는 **NAT 변환 테이블**을 사용하여 내부의 사설 IP 주소 및 포트 번호와 외부에서 사용하는 공인 IP 주소 및 새로운 포트 번호를 매핑한다.
    ...
    1. 내부 호스트(e.g. `10.0.0.1`, 포가 외부 웹 서버(예: `128.119.40.186`, 포트 `80`)로 요청을 보낸다.
...
NAT는 널리 사용되지만 몇 가지 비판점이 있다.

- **포트 번호의 용도**: 포트 번호는 호스트가 아닌 프로세스를 식별하는 데 사용되어야 한다는 원칙을 위반한다. 이로 인해 P2P 애플리케이션이나 외부에서 내부 서버로 접속해야 하는 경우 문제가 발생할 수 있다.
    
- **계층 구조 위반**: 라우터는 네트워크 계층(계층 3) 장치로서 IP 주소까지만 처리해야 하지만, NAT는 전송 계층(계층 4)의 포트 번호까지 수정한다. 이는 호스트 간의 직접적인 통신을 방해하는 요소로 작용할 수 있다.