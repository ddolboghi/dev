...
## NAT의 문제점
- **프로세스 주소 지정 위반**: 포트 번호는 호스트가 아닌 **프로세스를 식별**하는 데 사용되어야 한다는 원칙을 위반한다. 이로 인해 P2P 애플리케이션이나 외부에서 내부 서버로 접속해야 하는 경우 문제가 발생할 수 있다.
- **라우터의 역할 위반**: 라우터는 네트워크 계층(L3) 장치로서 IP 주소까지만 처리해야 하지만, NAT는 전송 계층(L4)의 포트 번호까지 수정한다. 이는 호스트 간의 직접적인 통신을 방해하는 요소로 작용할 수 있다.
# IPv6
1990년대 초, IETF는 32비트 IPv4 주소 공간이 고갈될 것이라는 예측에 따라 IPv6 개발을 시작했다.
## IPv6 데이터그램 형식
- **128비트로 확장된 주소**: IP 주소 크기를 32비트에서 **128비트**로 대폭 확장하여 주소 고갈 문제를 해결했다. 또한, 유니캐스트와 멀티캐스트 외에 **애니캐스트(anycast)** 라는 새로운 주소 유형을 도입하여 데이터그램이 특정 그룹 내의 한 호스트에게 전달될 수 있게 했다.
- **단순화된 40바이트 헤더**: IPv4의 여러 필드를 제거하거나 옵션으로 만들어 고정 길이 40바이트 헤더를 구현했다. 이를 통해 라우터에서의 IP 데이터그램 처리 속도를 높였다.
- **플로우 레이블링(Flow labeling)**: 송신자가 특별한 처리를 요청하는 특정 플로우에 속하는 패킷에 레이블을 지정하는 것이다. 20bit의 플로우 레이블 필드를 통해 특정 플로우에 속한 패킷들을 식별할 수 있다. 이는 실시간 서비스나 특정 QoS(서비스 품질)를 요구하는 트래픽을 처리하는 데 유용하다.

![[IPv6_datagram_format.png]]
### IPv6 헤더 필드
- **Version**: 4bit 필드로, IPv6의 경우 값이 6이다.
- **Traffic class**: 8bit 필드로, IPv4의 TOS 필드처럼 데이터그램의 우선순위를 지정하는 데 사용된다.
- **Flow label**: 20bit 필드로, 특정 플로우에 속한 데이터그램들을 식별한다.
- **Payload length**: 16bit 필드로, 40byte 고정 헤더를 제외한 데이터그램 페이로드의 길이를 byte 단위로 나타낸다.
- **Next header**: 페이로드에 담긴 데이터가 전달될 상위 프로토콜(e.g. TCP 또는 UDP)을 식별한다. IPv4의 프로토콜 필드와 동일한 값을 사용한다.
- **Hop limit**: 데이터그램을 전달하는 각 라우터가 1씩 감소시키며, 값이 0이 되면 데이터그램을 폐기한다. IPv4의 TTL 필드와 유사하다.
- **Source & Destination addresses (출발지 및 목적지 주소)**: 각각 128비트 길이의 주소이다.
- **Data**: 데이터그램의 페이로드 부분이다.
### IPv4에서 제거된 필드
- **Fragmentation/Reassembly (단편화/재조립)**: IPv6에서는 중간 라우터에서의 단편화를 허용하지 않고, 출발지와 목적지에서만 수행하도록 변경했다. 라우터가 처리해야 할 데이터그램이 너무 크면, 해당 데이터그램을 폐기하고 "Packet Too Big" ICMP 오류 메시지를 송신자에게 보낸다. 이를 통해 네트워크 내 라우터의 IP 전달 속도를 크게 향상시켰다.
- **Header checksum**: 전송 계층과 링크 계층에서 이미 오류 검사를 수행하므로, 네트워크 계층에서의 체크섬 기능은 중복으로 간주되어 제거되었다. 이 또한 라우터의 처리 속도를 높이는 데 기여했다.
- **Options**: 표준 IP 헤더에서 제거되었지만, 'Next header' 필드를 통해 확장 헤더로 포함될 수 있다. 이로 인해 IP 헤더는 40바이트 고정 길이를 갖게 되었다.
## IPv4에서 IPv6로의 전환
모든 인터넷 장비를 한 번에 IPv6로 전환하는 것은 불가능하므로, **터널링(tunneling)** 이라는 점진적인 전환 방식이 채택되었다.
1. IPv6 노드 사이에 IPv4 라우터들이 존재하는 경우, 출발지 IPv6 노드는 전체 IPv6 데이터그램을 IPv4 데이터그램의 페이로드 부분에 넣는다.
2. 이 IPv4 데이터그램은 터널의 반대편 IPv6 노드를 목적지로 하여 전송된다.
3. 중간의 IPv4 라우터들은 이것을 일반적인 IPv4 데이터그램으로 처리한다.
4. 목적지 IPv6 노드는 IPv4 데이터그램을 수신하면, 페이로드에서 IPv6 데이터그램을 추출하여 정상적으로 처리한다.