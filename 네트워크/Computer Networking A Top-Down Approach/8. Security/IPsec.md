...
- 목적: IPsec 개체 간 인증 수행, SA 자동 생성 및 협상, 필요한 암호화 키 교환을 자동화한다.
- 동작 방식: IKE는 SSL(TSL) handshake와 유사하다. 각 IPsec 개체는 공개키가 포함된 인증서를 가지고 있으며, 이를 통해 서로를 인증하고 보안 채널을 형성하여 SA를 설정한다.