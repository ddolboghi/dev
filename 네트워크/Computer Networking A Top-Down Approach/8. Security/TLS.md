...
2. **알고리즘 선택 및 인증서 전송**: 서버는 목록에서 대칭 키 알고리즘(e.g. AES), 공개 키 알고리즘(e.g. RSA), HMAC 알고리즘(e.g. SHA-1) 등을 선택한다. 그리고 자신의 선택 결과와 함께 인증서와 **서버 nonce**를 클라이언트에 보낸다.
3. **Pre-Master Secret(PMS) 전송**: 클라이언트는 인증서를 검증하고 서버의 공개 키를 추출한다. 그리고 **PMS**를 생성하여 서버의 공개 키로 암호화한 뒤 서버에 전송한다.
4. **Master Secret 계산**: 클라이언트와 서버는 PMS와 교환된 nonce들을 사용하여 **독립적으로 MS를 계산**한다. 이 MS로부터 2개의 암호화 키와 2개의 HMAC 키를 생성한다. CBC 방식의 대칭 암호를 사용할 경우 2개의 초기화 벡터(IV)도 함께 생성한다. 이후 모든 메시지는 암호화되고 인증된다.
5. **handshake 무결성 확인:**
    - 클라이언트는 모든 handshake 메시지의 HMAC을 전송한다.
    - 서버도 모든 handshake 메시지의 HMAC을 전송한다.
### handshake 보호의 중요성
- 마지막 두 단계(5, 6번)는 핸드셰이크 과정이 조작되지 않았음을 보장한다.
	- 예를 들어, 클라이언트가 강력한 알고리즘과 약한 알고리즘을 모두 지원한다고 목록을 보냈을 때, 중간자가 강력한 알고리즘을 삭제하여 강제로 약한 알고리즘을 사용하게 만들 수 있다.
- 이러한 공격을 방지하기 위해 클라이언트와 서버는 handshake의 마지막에 그들이 주고받은 **모든 handshake 메시지의 MAC**을 교환하고 비교한다.
- 만약 중간자가 알고리즘 목록을 수정했다면, 마지막에 계산된 MAC 값이 서로 달라지게 되어 불일치가 감지되고 연결이 종료된다.
## 연결 종료
- TCP 연결을 끊을 때, 단순히 TCP FIN 세그먼트를 보내는 것만으로는 보안상 취약하다(truncation attack). **공격자가 조기에 FIN을 보내 연결을 끊을 수 있기 때문**이다.
- TLS는 type 필드에 **종료가 명시된 TLS 레코드**를 전송하여 연결을 종료한다.
- 수신 측은 이 보안된 TLS 종료 레코드를 받았을 때만 데이터 전송이 끝났음을 확신하고 TCP 연결을 종료한다.