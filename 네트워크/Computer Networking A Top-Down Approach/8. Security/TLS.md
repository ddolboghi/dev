...
4. **Master Secret 계산**: 클라이언트와 서버는 PMS와 교환된 nonce들을 사용하여 독립적으로 **MS**를 계산한다. 이 MS로부터 2개의 암호화 키와 2개의 HMAC 키를 생성한다. CBC 방식의 대칭 암호를 사용할 경우 2개의 초기화 벡터(IV)도 함께 생성한다. 이후 모든 메시지는 암호화되고 인증된다.
5. **handshake 무결성 확인:**
...
- 이러한 공격을 방지하기 위해 클라이언트와 서버는 핸드셰이크의 마지막에 그들이 주고받은 **모든 handshake 메시지의 MAC**을 교환하고 비교한다.
