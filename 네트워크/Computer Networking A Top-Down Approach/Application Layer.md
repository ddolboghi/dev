...
### HTTP/2 Framing
- HTTP/2는 HOL blocking을 피하기 위해 메시지를 작은 프레임으로 쪼개고, 동일한 TCP 연결에 교차 배치(interleave)시킨다.
- 예를 들어, 웹 페이지가 1000프레임짜리 비디오와 2프레임짜리 객체 8개로 이뤄지고, 모든 프레임들의 길이가 고정되있을때, 첫번째 비디오 프레임을 보낸 후, 각 객체의 첫번째 프레임을 보낸다. 그러므로 모든 객체(2프레임짜리)는 18 프레임을 보낸 후에 보내진다.
145p부터
