...
- 수신자가 메시지 작성을 마치면, 수신자의 사용자 에이전트는 메시지를 수신자의 메일 서버로 전송하고, 메시지는 메일 서버의 **outgoing message queue**에 저장된다. 송신자가 메시지를 읽고 싶을 때, 송신자의 사용자 에이전트는 송신자의 메일 서버에 있는 **mailbox**에서 메시지를 검색한다.
- 메일 서버는 이메일 인프라의 핵심으로, 
