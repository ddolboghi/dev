...
# DNS
인터넷 호스트는 호스트 이름과 IP 주소라는 두 가지 식별자를 가집니다. 사람들은 기억하기 쉬운 호스트 이름을 선호하지만, 라우터는 계층 구조를 가진 고정 길이의 IP 주소를 선호합니다. 이 두 가지 선호도를 조화시키기 위해 호스트 이름을 IP 주소로 변환하는 디렉터리 서비스가 필요한데, 이것이 바로 인터넷의 **도메인 이름 시스템(Domain Name System, DNS)**의 주요 임무입니다.
### DNS에 의해 제공되는 서비스
