...
- 송신자가 메시지 작성을 마치면, 송신자의 사용자 에이전트는 메시지를 송신자의 메일 서버로 전송하고, 메시지는 메일 서버의 **outgoing message queue**에 저장된다. 수신자가 메시지를 읽고 싶을 때, 수신자의 사용자 에이전트는 수신자의 메일 서버에 있는 **mailbox**에서 메시지를 검색한다.
- 메일 서버는 이메일 인프라의 핵심으로, 각 수신자는 메일 서버들 중 하나에 있는 mailbox를 가진다.
- 송신자의 메일 서버가 수신자의 메일 서버로 메일을 전달하지 못하면, 송신자의 메일 서버는 메시지 큐에 메일을 저장하고 나중에 다시 보내기를 시도한다.
- SMTP는 다른 애플리케이션 계층의 프로토콜처럼 클라이언트 사이드(수신자의 메일 서버)과 서버 사이드(송신자의 메일 서버)를 가지며, 모든 메일 서버가 두 사이드를 가진다.
- 메일 서버가 다른 메일 서버로 메일을 보낼때 SMTP 클라이언트로서 작동하고, 메일 서버가 메일을 받을때는 SMTP 서버로서 작동한다.
- SMTP는 **TCP**를 기본 전송 프로토콜로 사용하며, **포트 번호 25**를 통해 동작한다.
### SMTP 클라이언트 - 서버 상호작용
- **SMTP는 중간 서버를 거치지 않고**, 클라이언트 측 서버와 서버 측 서버가 **직접 통신**한다.
- SMTP는 클라이언트 메일 에이전트와 서버 메일 에이전트 간의 명령/응답 상호작용을 정의한다.
- SMTP 클라이언트는 ASCII 명령을 메일 서버로 보내고, 메일 서버는 ASCII 응답을 돌려준다.
- SMTP 트랜스포트를 거친 후에 ASCII 메시지는 바이너리 데이터로 디코딩된다.
1. 발신자가 사용자 에이전트를 이용해서 이메일을 보낸다.
2. 사용자 에이전트는 발신자의 메일서버로 메시지(이메일)를 보내고 메시지 큐에 넣는다.
3. 발신자의 메일 서버에서 동작하는 SMTP 클라이언트는 메시지 큐에 있는 메시지를 보고, 수신의 메일 서버에서 동작하는 SMTP 서버와 TCP 연결을 한다.
4. SMTP 초기 handshaking 후, SMTP 클라이언트는 발신자의 메시지(이메일)을 TCP 연결로 보낸다.
5. 수신자의 메일 서버(SMTP 서버)는 메시지를 받아 수신자의 mailbox안에 넣는다.
6. 수신자는 사용자 에이전트를 통해 메시지를 읽는다.
