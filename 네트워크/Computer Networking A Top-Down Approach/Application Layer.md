...
### DNS에 의해 제공되는 서비스
DNS는 계층 구조의 DNS 서버로 구현된 분산 데이터베이스이자, 호스트가 이 분산 데이터베이스를 쿼리할 수 있게 하는 애플리케이션 계층 프로토콜이다. 
...
이 과정에서 DNS는 추가적인 지연을 발생시킬 수 있지만, 일반적으로 원하는 IP 주소는 근처 DNS 서버에 캐시되어 있어 평균 DNS 지연과 네트워크 트래픽을 줄이는 데 도움이 된다.
