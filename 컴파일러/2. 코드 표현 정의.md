...
## 표현식 문제
- 컴파일러에서 표현식 구문 트리 노드를 모델링하는 방법은 언어의 성격에 따라 달라진다.
- 객체 지향 언어는 인터프리터 패턴을 이용하여 새로운 표현식을 추가하기 쉽지만, 새로운 추상 메서드를 추가하려면 모든 표현식에서 이를 구현해야 한다.
- 함수형 언어는 함수의 바디에 패턴 매칭을 사용해서 원하는 동작을 모든 타입(표현식)에 걸쳐 한 번에 적용할 수 있어 새 동작을 추가하기 쉽지만, 새로운 타입을 추가하려면 모든 함수의 패턴 매칭에 새 케이스를 추가해야 한다.
## visitor 패턴
- visitor 패턴으로 표현식 문제를 해결할 수 있다.
- 이 패턴은 방문하는 행위와 무관하며, 트리 탐색과 무관하다.
- 이 패턴은 객체 지향 언어 내부에서 함수형 스타일을 흉내 낸 것이다.
...
```
- 모든 동물에 대한 작업은 이 인터페이스를 구현한 새 클래스에서 구현된다. 즉, 동물 종류마다 구체화한 메서드(concrete method)가 배치된다.
...
```
- 위 코드에서 어떤 동물이 주어졌을때, 그 타입에 따라 정확한 visit 메서드로 보내기위해 다형성을 사용했다.