...
> 실제로 컴포넌트 성능 개선할 수 있을때만 사용해라
# useCallback
- `useMemo`는 특정 결과값을 재사용할때 사용
- `useCallback`은 특정 **함수**를 재사용할때 사용
- 함수 안의 state, props는 반드시 의존성 배열에 추가해야함
- 컴포넌트 렌더링 최적화 작업을 해줘야 성능이 최적화됨(`React.memo`등)
...
```
# React.memo
- 컴포넌트에서 리렌더링이 필요할때만 리렌더링하도록 설정
- 컴포넌트의 props가 바뀌지 않았다면 리렌더링 안함
